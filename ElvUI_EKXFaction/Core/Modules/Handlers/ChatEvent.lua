...
function ChatEvent:CallbackGuildMessage(inText, inSenderName, inLanguageName, _, inTargetName, inFlags, _, inChannelID, _, _, inLineID, inSenderGUID)
    EKX:Debug(LogCategory, "Guild message [%s][%s]", inText, inSenderName)
    ...
    --if(EKX.Player.GUID == inSenderGUID) then
        ...
        _NewMessage:SetFrom(EKX.Player.GuildName)
        ...
        _NewMessage:Print()
        EKX:Debug(LogCategory, inText)
        EKX.Network.Sender:SendMessage(_NewMessage)
    --end
end
