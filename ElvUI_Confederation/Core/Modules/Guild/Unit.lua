...
function Unit:Initialize(_Argument)
    ...
    if(self:IsPlayer()) then
        ...
        if(_Profession1ID ~= nil and _Profession1ID > 0) then
            ...
            if(CON.Professions:Contains(_Profession1) == false) then
                CON.Professions:AddProfession(_Profession1)
            end
            self:SetProfession1(CON.Professions:GetProfession(_Profession1:GetID()))
        else
        ...
        if(_Profession2ID ~= nil and _Profession2ID > 0) then
            ...
            if(CON.Professions:Contains(_Profession2) == false) then
                CON.Professions:AddProfession(_Profession2)
            end
            self:SetProfession2(CON.Professions:GetProfession(_Profession2:GetID()))
        else
...
function Unit:SetGUID(_GUID)
    ...
    self._GUID = _GUID
    self:IsPlayer(self:GetGUID() == CON.PlayerGUID)
    return self:GetGUID()
end
...
function Unit:SetZone(_Zone)
    assert(type(_Zone) == 'string')
    self._Zone = _Zone
    return self:GetZone()
end
