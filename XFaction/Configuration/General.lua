...
function XFG:SetupRealms()
	...
	for _, realm in XFG.Realms:SortedIterator() do
		...
		RealmXref[realm:GetName()] = XFG.Cache.Setup.Realms[#XFG.Cache.Setup.Realms]
		RealmXref[realm:GetID()] = XFG.Cache.Setup.Realms[#XFG.Cache.Setup.Realms]
			...
			XFG.Options.args.General.args.Setup.args.Guilds.args[tostring(4 * i + 2)] = {
				...
				set = function(info, value)
					if(string.len(value) > 0) then 
						...
						XFG.Cache.Setup.Guilds[i].initials = nil
					end
				end,
			}
			XFG.Options.args.General.args.Setup.args.Guilds.args[tostring(4 * i + 3)] = {
				...
				set = function(info, value)
					if(string.len(value) > 0) then
						...
						XFG.Cache.Setup.Guilds[i].name = nil
					end
				end,
			}
		end
			...
			XFG.Options.args.General.args.Setup.args.Teams.args[tostring(2 * i)] = {
				...
				set = function(info, value)
					if(string.len(value)) then
						...
						XFG.Cache.Setup.Teams[i].initials = nil
					end
				end,
			}
			XFG.Options.args.General.args.Setup.args.Teams.args[tostring(2 * i + 1)] = {
				...
				set = function(info, value)
					if(string.len(value)) then
						...
						XFG.Cache.Setup.Teams[i].name = nil
					end
				end,
			}
		end
						...
						Print = {
							...
							args = {
								...
								Order = {
									...
									disabled = function () return XFG.Config.Debug.Verbosity == 0 end,
									func = function(info) XFG.Orders:Print() end,
								},
								Player = {
									order = 19,
									type = 'execute',
